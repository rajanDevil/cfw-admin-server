//Change This//////////////////////////
const controllerPrefix = 'customerReturns';
const tableName = 'customer_returns';
const referenceTableID = 'sale_id';
/////////////////////////////////////
...
exports.getMany = async (req, res, next) => {
	try {
		...
		if (result.length > 0) {
			let totalItems=0;
			connection.query(`select count(*) as totalRows from `+tableName+` where `+referenceTableID+` in (?)`, [req.query.id],
			function (error, totalCounts){
				if(error) {
					return reject(error);
				}else{
