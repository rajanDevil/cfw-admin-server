//Edit Table Name////////////////
const tableName = 'vendor_returns';
////////////////////////////////
// const fs = require('fs');
...
exports.list = (reqParam) => {
	...
	if(reqParam.vendor_id || reqParam.q || reqParam.date_start || reqParam.date_end || reqParam.purchase_id || reqParam.product_id){
		...
		if(reqParam.vendor_id){
			if(!reqParam.date_start && !reqParam.date_end && !reqParam.q && !reqParam.product_id && !reqParam.purchase_id) {
		...
		if(reqParam.purchase_id){
			if(reqParam.vendor_id && !reqParam.date_end && !reqParam.q && !reqParam.date_start && !reqParam.product_id){
				queryBuilder += `purchase_id in (`+[reqParam.purchase_id]+`) `;
			}else if(!reqParam.vendor_id && !reqParam.product_id && !reqParam.date_end && !reqParam.q && !reqParam.date_start){
				queryBuilder += `purchase_id in (`+[reqParam.purchase_id]+`) `;
			}else{
		...
		if(reqParam.product_id){
			if((reqParam.purchase_id || reqParam.vendor_id) && !reqParam.date_end && !reqParam.q && !reqParam.date_start){
				queryBuilder += `product_id in (`+[reqParam.product_id]+`) `;
			}else if(!reqParam.purchase_id && !reqParam.vendor_id && !reqParam.date_end && !reqParam.q && !reqParam.date_start){
				queryBuilder += `product_id in (`+[reqParam.product_id]+`) `;
			}else{
		...
		if(reqParam.date_start){
			if((reqParam.purchase_id || reqParam.vendor_id || reqParam.product_id) && !reqParam.date_end && !reqParam.q){
				queryBuilder += `returned_at >= '`+[reqParam.date_start]+`' `;
			}else
			if(!reqParam.purchase_id && !reqParam.vendor_id && !reqParam.product_id && !reqParam.date_end && !reqParam.q) {		
				queryBuilder += `returned_at >= '`+[reqParam.date_start]+`' `;
			}else{
				queryBuilder += `returned_at >= '`+[reqParam.date_start]+`' AND `;
			}
		}

		//Date End Filter
		if(reqParam.date_end){
			if((reqParam.purchase_id || reqParam.vendor_id || reqParam.product_id || reqParam.date_start) && !reqParam.q){
				queryBuilder += `returned_at <= '`+[reqParam.date_end]+`' `;
			}else
			if(!reqParam.purchase_id && !reqParam.vendor_id && !reqParam.product_id && !reqParam.date_start && !reqParam.q) {		
				queryBuilder += `returned_at <= '`+[reqParam.date_end]+`' `;
			}else{
				queryBuilder += `returned_at <= '`+[reqParam.date_end]+`' AND `;
			}
		}
		...
		//Query Filter 
		if(reqParam.q){	
			queryBuilder += `(purchase_id LIKE '%`+reqParam.q+`%' OR 
			qty LIKE '%`+reqParam.q+`%' OR 
			variant LIKE '%`+reqParam.q+`%' OR 
			returned_at LIKE '%`+reqParam.q+`%' OR
