//Edit Table Name////////////////
const tableName = 'vendor_categories';
////////////////////////////////
...
exports.getMany = (reqParam) => {
	if(!reqParam._sort || !reqParam._order || !reqParam._start || !reqParam._skip){
		console.log("trying to get many");
		let idFilters= reqParam.id;
		
		console.log(idFilters);
		let theQuery = `select * from `+tableName+` where id in (`+idFilters+`)`;
		console.log(theQuery);
		return new Promise( async (resolve, reject) => {
			connection.query(`select * from `+tableName+` where id in (`+idFilters+`)`, function (error, result, fields) {
